...
class importacaoControle extends Controle {

    public function __construct(){

        // Configurações do componente
        $this->config = new stdClass();
        $this->config->componenteNome				= "Importação";
        $this->config->componenteUrl				= BASE . 'importacao';
        $this->config->componenteTitulo				= $this->config->componenteNome;
        $this->config->componenteSubTitulo			= '';
        $this->config->componenteTabela				= Sistema::Table("importacao");
        $this->config->pastaImagem                  = 'importacao';
        ...
        ,'item.arquivo'			=> array( 'name'=> 'Arquivo')
        ,'#acoes' 				=> array( 'name'=>'', 'size'=>'20px')
        );

        $this->config->campos = array('arquivo');
